...
/**
 ...
 input stream without problems.  Example:
...
public final class Scanner {
    ...
    private static final String SCANNER_REUSE_PROPERTY_NAME = "scanner.reuse";
    private static final String SCANNER_WORKING_DIR_PROPERTY_NAME = "scanner.workingdir";
    ...
    public static boolean getEcho() {
        try {
            return System.getProperty(SCANNER_ECHO_PROPERTY_NAME) != null;
    ...
    public static boolean getReuse() {
        try {
            return System.getProperty(SCANNER_REUSE_PROPERTY_NAME) != null;
        } catch (SecurityException e) {
            return false;
        }
    }
    ...
    public static String getWorkingDir() {
        try {
            return System.getProperty(SCANNER_WORKING_DIR_PROPERTY_NAME);
        } catch (SecurityException e) {
            return null;
        }
    }
    ...
    public Scanner(InputStream source) {
        ...
        if (source == System.in && getReuse()) {
            if (s_reader == null || s_lnReader == null) {
                _init(new InputStreamReader(source));
                s_reader = m_reader;
        ...
        m_echoInput = getEcho() && source == System.in;
    }
    ...
    private void _init(Reader source) {
    ...
    private String toPrintable(String str) {
        str = str.replace("\n", "\\n");
        str = str.replace("\t", "\\t");
        return str;
    }
    ...
    public String next() {
        ...
        try {
            ...
            String peekStr = null;
            while (true) {
                if (DEBUG) System.out.println("    Matcher 1");
                boolean endOfInput = !_hasNextChar();
                if (DEBUG) System.out.println("    Matcher 2a");
                peekStr = m_previousNextBuffer.toString();
                if (DEBUG) System.out.println("    Matcher 2b");
                if (!endOfInput) {
                    if (DEBUG) System.out.println("    Matcher 3");
                    peekStr += (char) _peek();
                    if (DEBUG) System.out.println("    Matcher 4");
                }

                if (DEBUG) System.out.println("    Matcher peekStr: \"" + toPrintable(peekStr) + "\"");
                Matcher matcher = m_delimiter.matcher(peekStr);
                if (matcher.find()) {
                    // determine whether we have a leading delimiter
                    if (DEBUG) System.out.println("    Matcher found a delimiter");
                    boolean leadingDelim = (matcher.start() == 0);
                    if (leadingDelim) {
                        // must also have a trailing delimiter or end-of-input to stop reading
                        if (DEBUG) System.out.println("    Matcher found leading delimiter");
                        int leadingEnd = matcher.end();
                        if (DEBUG) System.out.println("    Matcher looking for trailing delimiter starting at " + leadingEnd);
                        if (matcher.find()) {
                            if (DEBUG) System.out.println("    Matcher found trailing delimiter");
                            if (endOfInput || matcher.end() <= peekStr.length()) {
                                ...
                                if (DEBUG) System.out.println("    Matcher stopping; end of input or end != length (leading delim): " + returnValue);
                                break;
                            }
                        } else if (endOfInput) {
                            returnValue = peekStr.substring(leadingEnd);
                            if (DEBUG) System.out.println("    Matcher stopping at end of input; token to process: " + toPrintable(peekStr));
                        } else {
                            if (DEBUG) System.out.println("    Matcher didn't find trailing delimiter");
                        }
                    } else {
                        // no leading delimiter
                        if (DEBUG) System.out.println("    endOfInput: " + endOfInput + ", end: " + matcher.end() + ", peek length: " + peekStr.length());
                        if (endOfInput || matcher.end() <= peekStr.length()) {
                            // we have no leading delim, but we (greedily) found a trailing one; we can stop
                            returnValue = peekStr.substring(0, matcher.start());
                            if (DEBUG) System.out.println("    Matcher stopping; end of input or end != length (no leading delim): " + toPrintable(returnValue));
                            break;
                        }
                    }
                    ...
                    if (DEBUG) System.out.println("    Matcher stopping; token to process: " + toPrintable(peekStr));
                    returnValue = peekStr;
                }

                if (endOfInput) {
                    if (DEBUG) System.out.println("    Matcher stopping; end of input");
                    break;
                } else {
                    if (DEBUG) System.out.println("    Matcher gonna keep looking");
                }
            ...
            if (m_echoInput && !_inHasNextMethod()) {
                ...
                if (DEBUG) System.out.println("    previousNextBuffer: \"" + toPrintable(m_previousNextBuffer.toString()) + "\"");
                if (DEBUG) System.out.println("    echoing input: \"" + toPrintable(toPrint) + "\"");
    ...
    public String nextLine() {
        ...
        if (m_echoInput && returnValue.trim().length() > 0) {
    ...
    private boolean _hasNextChar() {
        try {
            int chr = _nextChar();
            if (DEBUG) System.out.println("    done with read");
            if (chr == EOF)
                return false;
